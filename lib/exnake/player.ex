defmodule Exnake.Player do
  ...
  def die(user_id), do: GenServer.cast(player_pid(user_id), {:die})

  def eat_food(user_id), do: GenServer.cast(player_pid(user_id), {:eat_food})
  ...
  defp kill_collided_players({player_states, collisions}) do
    players =
      Enum.map(player_states, fn state ->
  ...
  def handle_cast({:change_direction, direction}, %{dead: true} = state), do: {:noreply, state}
  ...
  def handle_cast({:eat_food}, %{dead: true} = state), do: {:noreply, state}

  def handle_cast({:eat_food}, state = %State{}) do
    {:noreply, Action.eat_food(state)}
  end

  def handle_call({:next_state}, _from, %{dead: true} = state), do: {:reply, state, state}
